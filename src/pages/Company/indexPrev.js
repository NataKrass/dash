...
const Card = styled.div`
  background: ${(props) => props.theme.colors.accentRed} url(${visa}) no-repeat
    6% 86%;
...
const Plan = styled(Block)`
  ...
  background: ${(props) => (props.current ? `#FFF9F9` : '')};
  .card-subtitle {
    font-size: 0.75rem;
    font-weight: 400;
  }
...
export default function Index() {
  ...
  useEffect(() => {
    ...
    /* eslint-enable */
  console.log('details?.payment_card_currency: ', details?.payment_card_currency, 'acive: ', active);
  useEffect(() => {
